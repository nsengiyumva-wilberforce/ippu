            ...
            <tr>
                ...
                <td>
                    <table class="no-space" style="width: 45%;margin-left: auto;">
                        ...
                        <tr>
                            <td>{{__('Number')}}:</td>
                            <td class="text-right">{{ sprintf("%05d", $invoice->invoice_id)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Issue Date')}}:</td>
                            <td class="text-right">{{ date('d M, Y',strtotime($invoice->issue_date))}}</td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="2">
                                <div class="view-qrcode">
                                    {!! DNS2D::getBarcodeHTML(url('invoice.link.copy',\Crypt::encrypt($invoice->invoice_id)), "QRCODE",2,2) !!}
                                </div>
                            </td>
                        </tr>
                        </tbody>
    ...
    <div class="invoice-body">
            ...
            <tr>
                <td>
                    ...
                    <p>
                        ...
                        {{!empty($customer->contact)?$customer->contact:''}}<br>
        ...
        <table class="add-border invoice-summary" style="margin-top: 30px;">
            ...
            <tr>
                ...
                <td>{{ ($invoice->totalRate)}}</td>
                <td>{{ ($invoice->totalDiscount)}}</td>
                <td>{{ ($invoice->totalTaxPrice) }}</td>
            ...
            <tr>
                ...
                <td colspan="2" class="sub-total">
                    <table class="total-table">
                        ...
                        <tr>
                            <td>{{__('Total')}}:</td>
                            <td>{{($invoice->getSubTotal()-$invoice->getTotalDiscount()+$invoice->getTotalTax())}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Paid')}}:</td>
                            <td>{{(($invoice->getTotal()-$invoice->getDue())-($invoice->invoiceTotalCreditNote()))}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Credit Note')}}:</td>
                            <td>{{(($invoice->invoiceTotalCreditNote()))}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Due Amount')}}:</td>
