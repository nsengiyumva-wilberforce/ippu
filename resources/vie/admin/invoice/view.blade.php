...
@section('breadcrumb')
    ...
    <div class="page-title-right">
        <ol class="breadcrumb m-0">
            ...
            <li class="breadcrumb-item"><a href="{{url('admin/invoices')}}">{{__('Invoices')}}</a></li>
                    ...
                    <div class="card-body">
                        <div class="row timeline-wrapper">
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                ...
                                {{-- @can('edit invoice') --}}
                                    <a href="{{ route('invoices.edit',\Crypt::encrypt($invoice->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="{{__('Edit')}}"><i class="las la-edit mr-2"></i>{{__('Edit')}}</a>
                                {{-- @endcan --}}
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                <div class="timeline-icons"><span class="timeline-dots"></span>
                                ...
                                @if($invoice->status==0)
                                    {{-- @can('send bill') --}}
                                        <a href="{{ url('admin/invoice/'.$invoice->id.'/sent') }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="{{__('Mark Sent')}}"><i class="ti ti-send mr-2"></i>{{__('Send')}}</a>
                                    {{-- @endcan --}}
                                @endif
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                ...
                                @if($invoice->status!=0)
                                    {{-- @can('create payment invoice') --}}
                                        <a href="#" data-url="{{ url('admin/invoice/'.$invoice->id.'/payment') }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" title="{{__('Add Payment')}}"><i class="ti ti-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                    {{-- @endcan --}}
                                @endif
                ...
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    @if(!empty($invoicePayment))
                        <div class="all-button-box mx-2 mr-2">
                            <a href="#" class="btn btn-sm btn-primary" data-url="{{ url('admin/invoice/'.$invoice->id.'/credit-note') }}" data-ajax-popup="true" data-title="{{__('Add Credit Note')}}">
                                {{__('Add Credit Note')}}
                            </a>
                        </div>
                    @endif
                   {{--  @if($invoice->status!=4)
                        <div class="all-button-box mr-2">
                            <a href="{{ url('invoice.payment.reminder',$invoice->id)}}" class="btn btn-sm btn-primary me-2">{{__('Receipt Reminder')}}</a>
                        </div>
                    @endif --}}
                    {{-- <div class="all-button-box mr-2">
                        <a href="{{ url('invoice.resent',$invoice->id)}}" class="btn btn-sm btn-primary me-2">{{__('Resend Invoice')}}</a>
                    </div> --}}
                               ...
                               {{--  @if(App\Models\Utility::getValByName('shipping_display')=='on')
                                ...
                                <div class="col">
                                    <div class="float-end mt-3">
                                        {!! DNS2D::getBarcodeHTML(url('invoice.link.copy',\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)), "QRCODE",2,2) !!}
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-3">
                            ...
                            @forelse($invoice->payments as $key =>$payment)
                                <tr>
                                    ...
                                    @can('delete invoice product')
                                        <td>
                                            <div class=" ms-2">
                                                {!! Form::open(['method' => 'post', 'url' => ['invoice.payment.destroy',$invoice->id,$payment->id],'id'=>'delete-form-'.$payment->id]) !!}

                                                <a href="#" class="mx-3 bg-danger btn btn-sm align-items-center bs-pass-para " data-bs-toggle="tooltip" title="Delete" title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$payment->id}}').submit();">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @can('edit credit note')
                                            <div class=" ms-2">
                                                <a data-url="{{ url('invoice.edit.credit.note',[$creditNote->invoice,$creditNote->id]) }}" data-ajax-popup="true" title="{{__('Edit')}}" title="{{__('Credit Note')}}" href="#" class="mx-3 bg-primary btn btn-sm align-items-center" data-bs-toggle="tooltip" title="{{__('Edit')}}">
                                        ...
                                        @can('delete credit note')
                                            <div class=" ms-2">
                                                {!! Form::open(['method' => 'DELETE', 'url' => array('invoice.delete.credit.note', $creditNote->invoice,$creditNote->id),'id'=>'delete-form-'.$creditNote->id]) !!}
                                                <a href="#" class="mx-3 bg-danger btn btn-sm align-items-center bs-pass-para " data-bs-toggle="tooltip" title="Delete" title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$creditNote->id}}').submit();">
                                                    <i class="las la-trash text-white"></i>
                                                </a>
                                                {!! Form::close() !!}
                                            </div>
...
{{--                                                <form method="post" action="{{ url('customer.invoice.payment',$invoice->id) }}" class="require-validation" id="payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <div class="row">--}}
{{--                                                        <div class="col-sm-8">--}}
...
{{--                                                <form class="w3-container w3-display-middle w3-card-4 " method="POST" id="payment-form" action="{{ url('customer.pay.with.paypal',$invoice->id) }}">--}}
{{--                                                    @csrf--}}
{{--                                                    <div class="row">--}}
{{--                                                        <div class="form-group col-md-12">--}}
{{--                                                            <label for="amount">{{ __('Amount') }}</label>--}}
...
{{--                                                <form class="w3-container w3-display-middle w3-card-4" method="POST" id="paystack-payment-form" action="{{ url('customer.invoice.pay.with.paystack') }}">--}}
{{--                                                    @csrf--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.flaterwave') }}" method="post" class="require-validation" id="flaterwave-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.razorpay') }}" method="post" class="require-validation" id="razorpay-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.mercado') }}" method="post" class="require-validation" id="mercado-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.paytm') }}" method="post" class="require-validation" id="paytm-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.mollie') }}" method="post" class="require-validation" id="mollie-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.skrill') }}" method="post" class="require-validation" id="skrill-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <form role="form" action="{{ url('customer.invoice.pay.with.coingate') }}" method="post" class="require-validation" id="coingate-payment-form">--}}
{{--                                                    @csrf--}}
{{--                                                    <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
...
{{--                                                <div class="card">--}}
{{--                                                    <form class="w3-container w3-display-middle w3-card-4" method="POST" id="paymentwall-payment-form" action="{{ url('invoice.paymentwallpayment') }}">--}}
{{--                                                        @csrf--}}
{{--                                                        <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">--}}
