...
//                    $billProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
                ...
                activity()->performedOn($bill)->log('edited Bill with ID:'.$bill->id);
    ...
    public function destroy(Bill $bill)
    {
        if(\Auth::user()->can('delete bill'))
        {
            if($bill->created_by == \Auth::user()->creatorId())
            {
                ...
                activity()->performedOn($bill)->log('deleted Bill with ID:'.$bill->id);
    ...
    public function sent($id)
    {
        if(\Auth::user()->can('send bill'))
        {
            ...
            activity()->performedOn($bill)->log('sent Bill with ID:'.$bill->id);
...
//
        ...
        if($setings['bill_resent'] == 1)
        {
            ...
            activity()->performedOn($bill)->log('resent Bill with ID:'.$bill->id);
        }
...
//                $billPayment->save();
            ...
            if($setings['new_bill_payment'] == 1)
            {
                ...
                activity()->performedOn($bill)->log('added payment to Bill with ID:'.$bill->id);
    ...
    public function paymentDestroy(Request $request, $bill_id, $payment_id)
    {
        ...
        if(\Auth::user()->can('delete payment bill'))
        {
            ...
            activity()->performedOn($bill)->log('deleted payment for Bill with ID:'.$bill->id);
