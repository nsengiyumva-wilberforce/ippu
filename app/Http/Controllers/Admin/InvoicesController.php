...
class InvoicesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // {
            ...
            $query = Invoice::where('created_by', '>=', 0);
...
//                $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
        ...
        {
            $pro_quantity = $invoiceProduct->quantity;

            // if($type == 'minus')
            // {
                $product->quantity = $pro_quantity - $invoiceProduct->quantity;
            // }
            // else
            // {
            ...
            activity()->performedOn($invoice)->log('created invoice:'.$invoice->id);
...
//                     }
                ...
                activity()->performedOn($invoice)->log('updated invoice:'.$invoice->id);
    ...
    public function payment($invoice_id)
    {
        ...
        // {
            ...
            return view('admin.invoice.payment', compact('customers', 'categories', 'invoice'));
        // }
        // else
        // {
        //     return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function invoice($invoice_id)
    {
        ...
        foreach($invoice->items as $product)
        {
            ...
            if(!empty($item->tax))
            {
                foreach($taxes as $tax)
                {
                    ...
                    $taxPrice = (($item->price*$item->quantity) - $item->discount) * ($tax->rate /100);
                    $totalTaxPrice += $taxPrice;
        ...
        if($invoice)
        {
            ...
            return view('admin.invoice.templates.template1', compact('invoice', 'color', 'settings', 'customer', 'img', 'font_color', 'customFields'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
